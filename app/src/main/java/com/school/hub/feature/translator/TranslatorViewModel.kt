...
class TranslatorViewModel(val repo: TranslatorRepository, private val images: ImageStorage) : ViewModel() {
    ...
    fun processBitmap(bmp: Bitmap) {
        ...
        viewModelScope.launch {
            runCatching {
                ...
                val translated = blocks.map { b -> b.copy(translated = runCatching { repo.translate(b.text, src, target) }.getOrDefault(b.text)) }
                camera = CameraResult(bmp, translated)
                refreshDownloaded()
