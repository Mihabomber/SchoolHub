...
object LlamaBridge {
    val isAvailable: Boolean by lazy { BuildConfig.WITH_LLAMA && runCatching { System.loadLibrary("schoolhub_llama"); true }.getOrDefault(false) }
...
enum class ChatFormat(private val systemSuffix: String = "") {
...
class LlmEngine {
    ...
    suspend fun load(modelId: String, file: File, format: ChatFormat, contextSize: Int): Boolean = withContext(dispatcher) {
        ...
        unloadInternal()
        this@LlmEngine.contextSize = contextSize
    ...
    suspend fun loadVision(mmproj: File): Boolean = withContext(dispatcher) {
        if (handle == 0L || !mmproj.exists()) return@withContext false
    ...
    suspend fun reset(): Boolean = withContext(dispatcher) {
        if (handle == 0L) return@withContext false
        stopRequested = false
        val ok = LlamaBridge.nativeReset(handle)
        if (!ok) throw IllegalStateException("Не удалось сбросить состояние модели.")
        history.clear(); firstTurn = true; ok
    }
    ...
    private fun fitContext(system: String?, user: String): String {
        val budget = (contextSize * .75).toInt() - 1024
        var result = user
        while (estimate(system.orEmpty()) + estimate(result) + history.sumOf { estimate(it.first) + estimate(it.second) } > budget && history.isNotEmpty()) history.removeAt(0)
        if (estimate(system.orEmpty()) + estimate(result) + history.sumOf { estimate(it.first) + estimate(it.second) } > budget) result = truncateMiddle(result, ((budget - estimate(system.orEmpty()) - history.sumOf { estimate(it.first) + estimate(it.second) }) * 3.5).toInt().coerceAtLeast(32))
        return result
    }
    private fun buildDelta(system: String?, user: String, first: Boolean): String {
        if (useTemplate) { val msgs = history.map { it.first to it.second } + ("user" to user); return LlamaBridge.nativeApplyTemplate(handle, msgs.map { it.first.toByteArray() }.toTypedArray(), msgs.map { it.second.toByteArray() }.toTypedArray(), true).toString(Charsets.UTF_8) }
        return format.turn(system, user, first)
    }

    fun generate(system: String?, user: String): Flow<String> = flow {
        check(handle != 0L) { "Модель не загружена" }
        val safeUser = fitContext(system, user)
        stopRequested = false
        val rc = LlamaBridge.nativeStartTurn(handle, buildDelta(system, safeUser, firstTurn).toByteArray())
        if (rc != 0) throw IllegalStateException(if (rc == -1) "Слишком длинный запрос для контекста модели." else if (rc == -3) "Генерация остановлена." else "Не удалось обработать запрос модели.")
        val answer = StringBuilder()
        while (!stopRequested) { val bytes = LlamaBridge.nativeNext(handle) ?: break; val part = bytes.toString(Charsets.UTF_8); answer.append(part); val stop = ChatFormat.STOPS.map { answer.indexOf(it) }.filter { it >= 0 }.minOrNull(); if (stop != null) { answer.setLength(stop); break }; emit(answer.toString()) }
        if (!stopRequested && answer.isNotEmpty()) { history += safeUser to answer.toString(); firstTurn = false }
    }.flowOn(dispatcher)

    fun generateWithImage(system: String?, user: String, image: Bitmap): Flow<String> = flow {
        check(handle != 0L) { "Модель не загружена" }; check(visionReady) { "Зрение модели не загружено" }; stopRequested = false
        val scaled = image; val rgb = ByteArray(scaled.width * scaled.height * 3); val pixels = IntArray(scaled.width * scaled.height); scaled.getPixels(pixels, 0, scaled.width, 0, 0, scaled.width, scaled.height)
        for (i in pixels.indices) { val c = pixels[i]; rgb[i * 3] = (c shr 16).toByte(); rgb[i * 3 + 1] = (c shr 8).toByte(); rgb[i * 3 + 2] = c.toByte() }
        val prompt = (system.orEmpty() + "\\n" + user).toByteArray(); val rc = LlamaBridge.nativeVisionTurn(handle, prompt, rgb, scaled.width, scaled.height)
        if (rc != 0) throw IllegalStateException("Не удалось обработать изображение моделью.")
        emitAll(stream())
    }.flowOn(dispatcher)
